...
import {
	...
	getNDayFromNow,
	getNDayFromNowInVN,
	newDateWithoutLocation,
} from "utilities/datetime";
import { getLastUpdateReport, getWaitTimeChartData } from "services/ReportService";
...
//Func only use for report
export const isEqualDateMonth = (dateInServer, dateInClient) => {
	let date1InVN = new Date(dateInServer);
	let date2InVN = new Date(dateInClient);
	const OFFSET_TIMEZONE_2 = date2InVN.getTimezoneOffset();
	date2InVN.setHours(date2InVN.getHours() - OFFSET_TIMEZONE_2 / 60 - 420 / 60);
	return date1InVN.getTime() == date2InVN.getTime();
};
...
const title = "common:menu.chat.wait_by_date";
const breadCrumbs = (t) => [
	{
		link: "/",
...
const WaitTime = ({ rowsPerPageOptions, ...props }) => {
	...
	useEffect(() => {
		topics.map((item) => {
			...
			item.waitTime = diffMins(item.approvalTime, item.createdTimeWithWorkingHour);
		});
	...
	const exportExcelFunc = async () => {
		...
		topicsTemp.forEach((item) => {
			...
			itemTemp["wait-time"] = item.waitTime || 0;
			itemTemp["short-status"] = conversationStatusLabel(t)[item.status];
			dataExportTemp.push(itemTemp);
		});
		setIsLoading(false);
		exportExcel(dataExportTemp, "Report", "WaitTime", headCells(t));
	};
	...
	useEffect(() => {
		...
		for (let i = new Date(start); i <= end; i.setDate(i.getDate() + 1)) {
			...
			const findDataDate = dataTemp.find((item) => isEqualDateMonth(item.countDate, i));
			dataChartFullItem.name = getFormattedDate(new Date(i));
			dataChartFullItem.value = findDataDate?.averageTime || 0;
		...
		<ReportApp breadcrumb={breadCrumbs(t)}>
			...
			<Box sx={{ paddingBottom: 0 }} width={"100%"} flexGrow={1}>
				...
				>
					<Filter
						scheduleTopic={
							SCHEDULE_TOPIC.COUNT_TOPIC_AND_GET_WAIT_TO_PROCESS_TIME_BY_HOUR
						}
				...
				<BarChart1 chartData={chartData} label={t("chat:waitingTime")} />
				<ConversationsTable
...
const getData = async (context, user) => {
	...
	const getLastUpdateReportRes = await getLastUpdateReport({
		ctx: context,
		params: {
			topic: SCHEDULE_TOPIC.COUNT_TOPIC_AND_GET_WAIT_TO_PROCESS_TIME_BY_HOUR,
		},
	});
	...
	const getChartDataRes = await getWaitTimeChartData({
		ctx: context,
		params: {
			customerRegionCode,
			customerSupportIDs,
	...
	if (getChartDataRes.data?.status == APIStatus.OK) {
		...
		const getListDataRes = await getListNumberByDate({
			...
			params: {
				...
				lastGenerateTime: new Date(lastUpdateReport),
				limit: rowsPerPage,
...
export default function WaitTimeReportPage(props) {
	return renderWithLoggedInUser(props, WaitTime);
}
