...
import { getCompletedTimeChartData, getLastUpdateReport } from "services/ReportService";
...
const CompletedTime = ({ rowsPerPageOptions, ...props }) => {
	...
	useEffect(() => {
		...
		for (let i = new Date(start); i <= end; i.setDate(i.getDate() + 1)) {
			...
			const findDataTime = dataTemp.find((item) => isEqualDateMonth(item.countDate, i));
			dataChartFullItem.name = getFormattedDate(new Date(i));
			dataChartFullItem.value = findDataTime?.averageProcessingTime || 0;
	...
	useEffect(() => {
		topics.map((item) => {
			...
			item.waitTime = diffMins(item.completedTime, item.createdTimeWithWorkingHour);
		});
	...
	const getListTopic = async (filterData) => {
		const dataQueryUrl = {
			...
			supporter: filterData.customerSupportIDs?.join(",") || "",
	...
	const exportExcelFunc = async () => {
		...
		topicsTemp.forEach((item) => {
			...
			itemTemp["completed-time"] = item.completedTime ? new Date(item.completedTime) : "";
			itemTemp["wait-time"] = item.waitTime == 0 ? 0 : item.waitTime ? item.waitTime : "";
			dataExportTemp.push(itemTemp);
		});
		setIsLoading(false);
		exportExcel(dataExportTemp, "Report", "CompletedTime", headCells(t));
	};
	...
	return (
		<ReportApp breadcrumb={breadCrumbs(t)}>
			...
			<Box sx={{ paddingBottom: 0 }} width={"100%"} flexGrow={1}>
				...
				>
					<Filter
						scheduleTopic={SCHEDULE_TOPIC.COUNT_COMPLETED_TOPIC}
				...
				<BarChart1 chartData={chartData} label={t("common:time.minute")} />
				<ConversationsTable
...
const getData = async (context, user) => {
	...
	let customerSupportIDs = query.supporter?.split(",") || [];
	...
	const getChartDataRes = await getCompletedTimeChartData({
		ctx: context,
		params: {
			customerRegionCode,
			customerSupportIDs,
...
export default function CompletedTimeReportPage(props) {
	return renderWithLoggedInUser(props, CompletedTime);
}
