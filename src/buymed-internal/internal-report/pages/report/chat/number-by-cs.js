...
import { getNumberByCSChartData } from "services/ReportService";
...
import BarChart2 from "components/ChatReport/Chart/BarChart2";
...
const title = "common:menu.chat.number_by_cs";
const breadCrumbs = (t) => [
	{
		link: "/",
...
const NumberByCS = ({ rowsPerPageOptions, ...props }) => {
	...
	const exportExcelFunc = async () => {
		...
		setIsLoading(false);
		exportExcel(dataExportTemp, "Report", "NumberByCS", headCells(t));
	};
	...
	useEffect(() => {
		...
		dataTemp.map((item) => {
			...
			dataChartItem.name =
				item.customerSupportID != -1 ? item.customerSupportName : t("common:not_found");
			dataChartItem.value = item.count;
			dataChartItem.id = item.customerSupportID;
			dataChartFull.push(dataChartItem);
		});
		dataChartFull.sort((a, b) => a.id - b.id);
		if (props.page) {
			setPage(props.page);
		} else {
			if (page == 1) {
...
const getData = async (context, user) => {
	...
	const getChartDataRes = await getNumberByCSChartData({
		ctx: context,
		params: {
			customerRegionCode,
			customerSupportIDs,
...
export default function NumberByCSReportPage(props) {
	return renderWithLoggedInUser(props, NumberByCS);
}
