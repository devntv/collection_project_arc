...
function render({ menuBar }) {
    const breadcrumb = [
        ...
        {
            name: "Cập nhật tab cho danh sách menu",
        },
    ];
    ...
    const index = +router.query?.index || 0;
    ...
    async function submit(data) {
        try {
            ...
            if (resp.status === "OK") {
                ...
                router.push({
                    pathname: "/marketing/menu-bar/edit",
                    query: { index },
                });
                return;
            }
    ...
    return (
        <>
            <Head>
                <title>Cập nhật tab cho danh sách menu</title>
            </Head>
            <AuthorizationScreen>
                <AppMarketing
...
export default function MenuBarEdit(props) {
