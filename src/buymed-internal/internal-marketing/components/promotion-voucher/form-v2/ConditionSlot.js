...
import { conditionLabel, defaultCondition } from '../constant';
...
export default function ConditionSlot(props) {
    ...
    const { control, errors, register, getValues, setValue, handleSubmit, setError, clearErrors } = hookForm
    ...
    useEffect(() => {
        if (promotionType !== "GIFT") clearErrors(sellerCode)
    }, [promotionType])
    ...
    return (
        <Grid item container spacing={1} xs={12}>
            ...
            <Grid item md={3} xs={12}>
                <Typography>
                    Nhà bán hàng {promotionType === "GIFT" && (<span style={{ color: "red" }}>*</span>)}
                </Typography>
                ...
                <MuiSingleCustom
                    ...
                    placeholder={promotionType !== "GIFT" ? "Tất cả" : "Chọn"}
                    options={sellerOpts}
                    control={control}
                    errors={errors}
                    onFieldChange={searchSeller}
                    required={promotionType === "GIFT"}
                    // disabled={disabled}
            ...
            <Grid item md={2} xs={12}>
                ...
                <TextField
                    ...
                    onChange={(e) => {
                        setValue("minQuantity", +e.target?.value || null)
