...
export default function UnavailableCustomer(props) {
    ...
    const handleAddCustomer = async (customerIds, isImport = false) => {
        if (customerIds.length > 0) {
            const resp = await getVoucherClient().createUserVoucher({
                ...
                status: UserPromotionStatusValue.INACTIVE,
            })
            if (resp.status === "OK") {
                toast.success(isImport ? "Import file thành công" : "Cập nhật khách hàng không được sử dụng thành công")
                setPage(0)
                ...
                if (isImport && customerIds.length > 0) {
                    let newList = [...listError, ...customerIds]
    ...
    const handleDeteleAllCustomer = async (status) => {
        const resp = await getVoucherClient().updateUserVoucher({
            voucherCode: voucher.code,
            status,
            prevStatus: UserPromotionStatusValue.INACTIVE,
            isUpdateAll: true
        })
        if (resp.status === "OK") {
            toast.success("Cập nhật khách hàng không được sử dụng thành công")
            setPage(0)
    ...
    return (
        <MyCard style={{ width: "100%" }}>
            <MyCardHeader title="Khách hàng không được sử dụng" style={{
            ...
            <MyCardContent>
                ...
                <ModalCustom
                    ...
                    maxWidth="xs"
                >
                    Bạn có chắc muốn xóa tất cả khách hàng không được sử dụng?
                </ModalCustom>
            </MyCardContent>
        </MyCard>
    )
