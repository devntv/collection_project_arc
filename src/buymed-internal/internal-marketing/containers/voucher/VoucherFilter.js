...
import { VoucherStatus, VoucherStatusText } from "view-model/promotion";
import { getCustomerClient } from "client/customer";
import MuiSingleAuto from "@thuocsi/nextjs-components/muiauto/single";
import { formSetter } from 'utils/HookForm';
import { ApplyTypeOpts } from "components/promotion-voucher/constant";
...
export const VoucherFilter = () => {
    ...
    const formDefaultValue = useMemo(() => {
        ...
        if (data.productCode && data.productCode !== "") {
            (async () => {
                const detail = await getProductClient().getListProductByCodes([data.productCode]);
                if (detail.status === "OK") {
                    formSetter(
                        {
                            productCode:
                            {
                                value: detail?.data[0]?.code || 0,
                                label: (detail?.data[0]?.productID || '') + ' - ' + (detail?.data[0]?.name),
                            }
                        },
                        [
                            "productCode",
    ...
    const { field: applyField } = useController({ control, name: "applyType" });
    ...
    const getData = async () => {
        ...
        const listProductDefaultReponse = await getProductClient().getProductListFromClient({
            offset: 0,
            limit: 20,
            search: "",
            ignoreCombo: true
        ...
        if (listProductDefaultReponse.status === "OK") {
            const list = listProductDefaultReponse.data?.map((product) => ({
                value: product.code,
    ...
    const handleApplyFilter = async (formData) => {
        ...
        if (qData.productCode) qData.productCode = qData.productCode.value
        router?.push({
            ...
            <MyCardActions>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} md={6} lg={4}>
                        ...
                        >
                            <MenuItem value="">Tất cả</MenuItem>
                            {Object.values(ApplyTypeOpts).map(({value, label}) => (
