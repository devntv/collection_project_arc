import { Typography, Grid, TextField, IconButton, InputAdornment } from '@material-ui/core'
import { useForm } from "react-hook-form";
import React, { useState, useEffect } from 'react'
import RemoveIcon from '@material-ui/icons/Remove';
import AddIcon from '@material-ui/icons/Add';
import { defaultCondition } from 'components/promotion-voucher/constant';
...
export default function CustomerConditionSlot(props) {
    ...
    const { control, errors, register, getValues, setValue, unregister, clearErrors } = hookForm
    let code = item.code;
                ...
                <Grid container spacing={2} alignItems="center" style={{
                ...
                }}>
                    {
                        listCustomerConditions.length !== 1 && (
                            <IconButton onClick={() => {
                                unregister(minDayNoOrder)
                                clearErrors(minDayNoOrder)

                                unregister(minOrderCount)
                                clearErrors(minOrderCount)

                                unregister(maxOrderCount)
                                clearErrors(maxOrderCount)
