...
import SuccessSubmittedForm from '@/modules/customer/SuccessSubmittedForm';
...
const schema = yup.object().shape({
  ...
  note: yup
    .string()
    .max(255, 'Bạn đã nhập quá vượt quá số kí tự quy định')
...
type Props = HTMLAttributes<HTMLDivElement> & {
  type: 'cooperate' | 'service';
  service?: 'onwheel' | 'sameday delivery' | 'fulfillment';
};

export default function CooperateServiceForm(props: Props) {
  ...
  const onSubmitHandler = (data: FormOnWheelBody) => {
    ...
    captcha.then((token) => {
      const payload = {
        ...
        service: props.service ? props.service : data.service,
      };
      submitData(payload);
    });
  };
        ...
        <div>
          {props.type === 'cooperate' ? (
            ...
            >
              ...
              <option value={props.service}>
                {
                  servicesCooperateFormData.find(
                    ({ value }) => value === props.service
                  )?.name
                }
              </option>
            </select>
