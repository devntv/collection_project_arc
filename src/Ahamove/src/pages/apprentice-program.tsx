...
import { ApprenticeProgramPage } from '@/modules/recruitment/apprentice-program/ApprenticeProgramPage';
import { recruitmentConfig } from '@/modules/recruitment/recruitmentConfig';

type Props = {
  // Declare Apprentice Program props
...
export default function ApprenticeProgram(
  _props: InferGetServerSidePropsType<typeof getServerSideProps>
) {
  return (
    <ApprenticeProgramPage
...
export const getServerSideProps: GetServerSideProps<Props> = async (
  ...
  try {
    await await getHelpCenterFn({
      type: 'internship-page',
    })
      .then((res) => {
  ...
  const queryInfiniteJobs: Partial<GetJobsParams> = {
    ...
    'filters[job_categories][id][$eq]': 144,
  };
  if (query?.category) {
