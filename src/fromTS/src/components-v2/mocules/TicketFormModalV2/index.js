...
import { EnumsTicket, ENUM_ORDER_STATUS_LABEL } from 'constants/Enums';
...
import InfoInput from '../../../components/mocules/InfoInput';
import SearchProduct from '../../../components/mocules/SearchProduct';
import UploadImages from '../../../components/mocules/UploadImages';
import styles from './styles.module.css';
import validateForm from './Validateform';

const TicketFormModalV2 = ({
  visible,
  onClose,
  orderId = 0,
  productName,
  ...
  const { reasonsTicket } = useSetting();
  ...
  const [curReason, setCurReason] = useState((reasonsByType && reasonsByType[0]) || null);
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (!isValid(feedbackResult)) throw new Error(feedbackResult.message || 'Gửi phản hồi thất bại');
      ...
      if (onSuccess) {
        onSuccess(getFirst(feedbackResult));
      }
      setVal({});
      gtag.sendFeedback();
    } catch (error) {
  ...
  const handleOnChangeImages = (imgs) => {
    setTimeout(() => {
      setVal({ ...val, isDisableSubmit: false, imageUrls: imgs });
    }, 1500);
  };
          ...
          <Grid item xs={12} container justifyContent="flex-start" spacing={1}>
            <InfoFormControl xs={12} md={12} label="Bạn muốn hỗ trợ về vấn đề gì" isRequired>
              <NativeSelect
                ...
                disabled={ticketType === EnumsTicket.TicketEnums.PRODUCT || !!orderId}
              >
          ...
          {curReason?.showBankAccount && [EnumsTicket.TicketEnums.ORDER].indexOf(curType) >= 0 && (
            <Grid item xs={12} container justifyContent="flex-start" spacing={1}>
              ...
              <Grid container alignContent="flex-start" direction="row" style={{ paddingTop: 5 }}>
                <Typography variant="body2" style={{ fontWeight: 500, paddingTop: 3 }}>
                  {val?.bankCode ? 'Bạn muốn thay đổi thông tin tài khoản ngân hàng?' : 'Bạn chưa có tài khoản ngân hàng? '}
                </Typography>
                <LinkComp href={MY_ACCOUNT} target="_blank" style={{ color: 'green' }} variant="body2">
...
export default TicketFormModalV2;
