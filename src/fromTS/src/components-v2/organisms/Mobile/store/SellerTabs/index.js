...
const SellerTabsMobile = ({ slug, isFlagship = false }) => {
      ...
      <Grid container>
        <Grid item xs={12} md={6} lg={6}>
          <Box className={styles.groupLink}>
            ...
            {user && isFlagship && (
              <LinkComp replace className={styles.link} href={linkListProduct}>
                <Typography className={isProductList ? clsx(styles.active, styles.linkTitle) : styles.linkTitle}>Sản Phẩm</Typography>
              </LinkComp>
            )}
            {!isFlagship && (
            ...
            {user && isFlagship && (
              <LinkComp replace className={styles.link} href={linkRewards}>
                <Typography className={isRewards ? clsx(styles.active, styles.linkTitle) : styles.linkTitle}>CT Trả Thưởng</Typography>
              </LinkComp>
            )}
            {/* Temporary hidden, system not support yet */}
            {/* {user && (
              <LinkComp className={styles.link} href="/">
                <Typography className={isRewards ? clsx(styles.active, styles.linkTitle) : styles.linkTitle}>CT Quà Tặng</Typography>
              </LinkComp>
            )} */}
          </Box>
        </Grid>
