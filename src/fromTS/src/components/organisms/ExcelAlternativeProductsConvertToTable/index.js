...
function ExcelAlternativeProductsConvertToTable({ rows = [] }) {
  return (
      ...
      <Table aria-label="simple table" stickyHeader>
        ...
        <TableBody>
          {rows?.map((row) => (
            ...
            >
              <TableCell
                colSpan={row.type === 'headerTitle' ? 3 : null}
                scope="row"
              ...
              >
                {row.type === 'headerTitle' || row.type === 'header' ? (
                  <Typography variant="subtitle2">{row?.dataRows?.name || ''}</Typography>
                ) : (
              ...
              {row.type === 'headerTitle' ? null : (
                <>
                  <TableCell className={clsx(row.type === 'headerTitle' && styles.headerTitle)} align="left">
                    {row?.type === 'header' || row?.type === 'headerTitle'
                      ? row.dataRows.volume
                      : (row.dataRows.volume && formatCurrency(row.dataRows.volume)) || ''}
                  </TableCell>
                  <TableCell className={clsx(row.type === 'headerTitle' && styles.headerTitle)} align="right">
                    {row?.type === 'header' || row?.type === 'headerTitle'
                      ? row?.dataRows.price
                      : (row?.dataRows?.price && formatCurrency(row?.dataRows?.price)) || ''}
                  </TableCell>
                  {/* <TableCell className={clsx(row.type === 'headerTitle' && styles.headerTitle)} align="left">
                    {row.dataRows.vol}
                  </TableCell> */}
...
export default ExcelAlternativeProductsConvertToTable;
