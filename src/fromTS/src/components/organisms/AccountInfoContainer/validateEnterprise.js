...
const validateEnterprise = (
  ...
  if (wardList?.length > 1 && (!wardCode || wardCode === DEFAULT_WARD_VALUE)) throw Error('Phường/Xã không được trống');
  ...
  // validate taxCode
  if (taxCode && ValidateUtils.validateMst(taxCode)) {
    throw Error(ValidateUtils.validateMst(taxCode));
  }
};

export default validateEnterprise;
